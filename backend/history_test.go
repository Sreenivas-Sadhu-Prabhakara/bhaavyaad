...
func TestPostgresStore_RoundTrip(t *testing.T) {
	...
	in, _ := json.Marshal(Purchase{Supplier: "ACME", Item: "soap", Price: 22, Qty: 10})
	saved, err := store.Save(Record{Input: in, Headline: 22, Label: "soap"})
	if err != nil {
		t.Fatalf("save: %v", err)
	}
	if saved.ID == 0 {
	...
	if len(items) == 0 || items[0].ID != saved.ID || items[0].Label != "soap" {
